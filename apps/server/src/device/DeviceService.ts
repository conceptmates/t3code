...
const HubStreamMode = Schema.Struct({
  ok: Schema.Boolean,
...
export const makeWithHosts = Effect.fn("DeviceService.makeWithHosts")(function* (
  ...
  /**
   * expo-device-hub streams every Android serial with grpc-screenshot, which
   * only emulators provide, so a physical device shows no video until its
   * stream switches to scrcpy. Switching restarts the stream for every viewer,
   * so an already-switched device is left alone.
   */
  const useScrcpyStream = Effect.fn("DeviceService.useScrcpyStream")(function* (
    ready: DeviceReadiness,
    deviceId: string,
  ) {
    const url = `${ready.hub.origin}${vendorPrefix("android")}/api/stream-mode?device=${encodeURIComponent(deviceId)}`;
    const current = yield* hubJson(HttpClientRequest.get(url), HubStreamMode, "attach stream").pipe(
      Effect.orElseSucceed(() => null),
    );
    if (current?.ok && current.mode === "scrcpy") return;
    const result = yield* HttpClientRequest.put(url).pipe(
      HttpClientRequest.bodyJson({ mode: "scrcpy" }),
      Effect.mapError(
        (cause) =>
          new DeviceOperationError({
      ...
      Effect.flatMap((request) => hubJson(request, HubStreamMode, "attach stream", BOOT_TIMEOUT)),
    );
    if (!result.ok) {
      return yield* new DeviceOperationError({
        operation: "attach stream",
  ...
  const open: DeviceService["Service"]["open"] = Effect.fn("DeviceService.open")(function* (input) {
    ...
    if (device.platform === "android" && device.physical) {
      yield* useScrcpyStream(ready, device.id);
    }
    if (hosts.get(host.id) !== host)
      return yield* new DeviceHostUnavailableError({
        hostId: host.id,
