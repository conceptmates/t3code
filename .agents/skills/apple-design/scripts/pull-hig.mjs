...
const HIG_ABSOLUTE_RE =
  /^https:\/\/developer\.apple\.com\/design\/human-interface-guidelines\/([a-z0-9-]+)(#.*)?$/;
...
const PLATFORM_LIST_RE = new RegExp(
  `^(${ALL_PLATFORMS.join("|")})(, (${ALL_PLATFORMS.join("|")}))*$`,
);
...
const CURATED_FILES = [
  {
    ...
    title: "Liquid Glass",
    covers:
      "Liquid Glass rules distilled from the materials and color pages, with implementation notes for Flutter, Tauri, Electron, and React Native",
  },
];
...
function parseArgs(argv) {
  ...
  return argv.reduce((opts, arg, index, all) => {
    ...
    if (key === "out" || key === "lookup" || key === "cache")
      return { ...opts, [key]: path.resolve(value) };
...
function listedPages(collection) {
  const identifiers = (collection.topicSections ?? []).flatMap(
    (section) => section.identifiers ?? [],
  );
  return identifiers
  ...
  const groups = await Promise.all(
    entries
      ...
      .map(async (group) => {
        ...
        return listedPages(sub).map((entry) => ({
          ...entry,
          sectionPath: [section.metadata.title, sub.metadata.title],
        }));
      }),
  );
...
function platformsMentioned(text) {
  return ALL_PLATFORMS.filter((token) =>
    new RegExp(`(^|[^A-Za-z])${token}([^A-Za-z]|$)`).test(text),
  );
}
...
function platformGroupLabel(kept) {
  const mobile =
    kept.includes("iOS") && kept.includes("iPadOS")
      ? "mobile"
      : kept.includes("iOS")
        ? "phone"
        : kept.includes("iPadOS")
          ? "tablet"
          : null;
...
function isDroppedBoilerplate(paragraph) {
  const match = paragraph
    .replace(/^\*+|\*+$/g, "")
    .match(/^No additional considerations for ([^.]+)\./);
  ...
  const walk = blocks.reduce(
    (state, block) => {
      if (block.type === "heading") {
        ...
        const entry = block.anchor
          ? [[block.anchor.toLowerCase(), dropped ? "" : githubSlug(relabelHeading(block.text))]]
          : [];
        const skipUntil = insideSkipped
          ? state.skipUntil
          : isDroppedPlatformText(block.text)
            ? block.level
            : null;
      ...
      if (block.type === "tabNavigator") {
        ...
        const nested = (block.tabs ?? []).flatMap((tab) =>
          anchorEntries(tab.content ?? [], skipped || isDroppedPlatformText(tab.title)),
        );
...
function anchorMapFor(json) {
  const content =
    json.primaryContentSections?.find((section) => section.kind === "content")?.content ?? [];
...
function renderImage(node, ctx) {
  ...
  const alt = (node.metadata?.abstract ?? [])
    .map((part) => part.text ?? "")
    .join("")
    .trim();
...
function collectReferences(block, ctx) {
  ...
  return nodes
    ...
    .map((node) => {
      ...
      return slug && ctx.included.has(slug) && slug !== ctx.slug
        ? { slug, title: ref.title }
        : null;
    })
    .filter(Boolean);
}
...
function renderAside(block, ctx, level) {
  const label =
    block.name ??
    (block.style ? block.style.charAt(0).toUpperCase() + block.style.slice(1) : "Note");
  ...
  return (cell ?? [])
    .map((block) =>
      block.type === "paragraph"
        ? renderInline(block.inlineContent, ctx)
        : renderBlocks([block], ctx, 6).join(" "),
    )
    .join("<br>")
    .replace(/\n+/g, " ")
    .replace(/\|/g, "\\|")
...
function renderTable(block, ctx) {
  const cells = resolveSpans(
    (block.rows ?? []).map((row) => row.map((cell) => cellText(cell, ctx))),
    block.extendedData,
  );
...
function renderTabs(block, ctx, level) {
  ...
  const rendered = tabs.map((tab) => {
    ...
    const body = trimBlankEdges(
      renderBlocks(repeatsTitle ? content.slice(1) : content, ctx, tabLevel),
    );
    return { title: tab.title, body };
  });
  const sections = rendered
    .filter((tab) => tab.body.length > 0)
    .flatMap((tab) => [headingLine(tabLevel, tab.title), "", ...tab.body, ""]);
  const imageOnly = rendered.filter((tab) => tab.body.length === 0).map((tab) => tab.title);
  const caption =
    imageOnly.length > 0 && tabs.length > 1 ? [`*Illustrated: ${imageOnly.join(", ")}.*`, ""] : [];
...
function renderRow(block, ctx, level) {
  return (block.columns ?? []).flatMap((column) =>
    (column.content ?? []).flatMap((child) =>
      child.type === "heading"
        ? [`**${relabelHeading(child.text)}**`, ""]
        : renderBlock(child, ctx, level),
    ),
  );
}
...
function renderBody(blocks, ctx) {
  ...
  const final = blocks.reduce((state, block) => {
    ...
    if (state.inResources) {
      return state.resourcesSub === "Related"
        ? { ...state, related: [...state.related, ...collectReferences(block, ctx)] }
        : state;
    }
...
function reduceHeading(state, block) {
  const unskipped =
    state.skipUntil !== null && block.level <= state.skipUntil
      ? { ...state, skipUntil: null }
      : state;
  if (unskipped.skipUntil !== null) return unskipped;
  if (block.level === 2 && block.text === "Resources")
    return { ...unskipped, inResources: true, resourcesSub: null };
  if (unskipped.inResources && block.level > 2) return { ...unskipped, resourcesSub: block.text };
  const outside = unskipped.inResources
    ? { ...unskipped, inResources: false, resourcesSub: null }
    : unskipped;
  if (isDroppedPlatformText(block.text)) return { ...outside, skipUntil: block.level };
  return {
    ...outside,
    level: block.level,
    lines: [...outside.lines, headingLine(block.level, block.text), ""],
  };
}
...
function coveredPlatformsLine(page) {
  if (page.platforms) {
    ...
    const note = dropped.length
      ? ` (guidance specific to ${prettyPlatforms(dropped)} omitted)`
      : "";
...
function renderPage(page, included, anchors, staleAnchors) {
  ...
  if (content.length === 0)
    throw new Error(
      `No content blocks for ${slug}; Apple's page format may have changed. Nothing was pruned.`,
    );
  ...
  const relatedLines = related.length
    ? [
        "## Related guidelines",
        "",
        ...uniqueBySlug(related).map((item) => `- [${item.title}](${item.slug}.md)`),
        "",
      ]
    : [];
  const header = [
  ...
  return (nodes ?? [])
    .map((node) => {
      ...
      if (node.type === "reference")
        return node.overridingTitle ?? refs[node.identifier]?.title ?? "";
...
function abstractOf(json) {
  return plainText(json.abstract, json.references ?? {})
    .replace(/\s+/g, " ")
    .trim();
}
  ...
  return pages.reduce((groups, page) => {
    ...
    if (existing)
      return groups.map((group) =>
        group.key === key ? { ...group, pages: [...group.pages, page] } : group,
      );
...
function renderLookup(included, omitted, pulledOn) {
  ...
  const curated = [
    ...
    "| --- | --- | --- |",
    ...CURATED_FILES.map(
      (item) => `| [${item.title}](hig/${item.file}) | \`${item.file}\` | ${escape(item.covers)} |`,
    ),
    "",
  ];
  const omittedTable = [
    ...
    "| --- | --- |",
    ...omitted.map(
      (page) => `| [${escape(page.title)}](${WEB_BASE}/${page.slug}) | ${escape(page.reason)} |`,
    ),
    "",
  ];
  const intro = [
...
async function main() {
  ...
  const crawled = (
    await Promise.all(SECTIONS.map((section) => crawlSection(section, opts)))
  ).flat();
  const entries = uniqueBySlug(crawled);
  if (entries.length === 0)
    throw new Error(
      "The crawl found no pages. Apple may have changed the HIG data format; nothing was written.",
    );
  ...
  if (included.length === 0)
    throw new Error(
      "Every crawled page was omitted. Check the platform metadata; nothing was written.",
    );
  ...
  await Promise.all(
    included.map((page) =>
      writeFile(
        path.join(opts.out, `${page.slug}.md`),
        renderPage(page, includedSet, anchors, staleAnchors),
      ),
    ),
  );
  ...
  const pruned = opts.prune
    ? await pruneStaleFiles(opts.out, keep, opts.forcePrune)
    : { removed: [], refused: [] };

  console.log(
    `Wrote ${included.length} guideline files to ${path.relative(REPO_ROOT, opts.out) || opts.out}`,
  );
  console.log(
    `Omitted ${omitted.length} pages (listed in ${path.relative(REPO_ROOT, opts.lookup) || opts.lookup})`,
  );
  if (staleAnchors.size)
    console.log(
      `Dropped ${staleAnchors.size} anchor(s) Apple no longer publishes: ${[...staleAnchors].join(", ")}`,
    );
  if (pruned.removed.length) console.log(`Pruned stale files: ${pruned.removed.join(", ")}`);
  if (pruned.refused.length) {
    console.warn(
      `Refused to prune ${pruned.refused.length} files in one run (${pruned.refused.join(", ")}). Re-run with --force-prune if Apple really removed them.`,
    );
  }
}
